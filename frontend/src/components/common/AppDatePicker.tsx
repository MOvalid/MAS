...
export const AppDatePicker = ({
  ...
  if (Platform.OS === 'web') {
    return (
      <View
        style={[styles.webContainer, webContainerStyle(colors, width), style]}
      >
        <input
          type="date"
          value={value}
  ...
  return (
    <View
      style={[styles.nativeContainer, nativeContainerStyle(colors, width), style]}
    >
      <TouchableOpacity
      ...
      >
        <AppText
          variant="bodyLarge"
          style={nativeTextStyle(colors, value)}
        >
          {value || placeholder}
        </AppText>
      </TouchableOpacity>
...
const nativeContainerStyle = (colors: any, width: string | number) => ({
  backgroundColor: colors.secondaryContainer,
  borderColor: colors.outlineVariant ?? colors.outline,
  width: width as DimensionValue,
});

const nativeTextStyle = (colors: any, value?: string) => ({
  color: value ? colors.onSecondaryContainer : colors.primary,
});

const webContainerStyle = (colors: any, width: string | number) => ({
  backgroundColor: colors.secondaryContainer,
  borderColor: colors.outlineVariant ?? colors.outline,
  width: width as DimensionValue,
});

const webInputStyle = (colors: any, fonts: any): React.CSSProperties => ({
  width: '80%',
  height: '100%',
  border: 'none',
